import DeviceNavigation from "../../components/device-navigation";
import { useNavigate, useParams } from "react-router-dom";
import { useSelector, useDispatch } from "react-redux";
import { Col, Table, Tab, Tabs, Spinner, Badge, Button } from "react-bootstrap";
import { FaCogs, FaTrash, FaDoorClosed, FaPlay, FaStop } from "react-icons/fa";
import { MdCheck, MdClose, MdDynamicFeed } from "react-icons/md";
import { getAddresses, getDhcp, removeDhcp, getPools, getDNS, removeAddresses, removePools } from "../../store/ip/methods";
import { reset } from '../../store/ip/slice';
import { useEffect } from "react";
import CreateIP from "./components/ip-create";
import DeleteItem from "./components/ip-delete";


const IP = () => {
    const { uuid } = useParams();
    const navigate = useNavigate();
    const dispatch = useDispatch();

    const { user, token } = useSelector(
        (state) => state.auth
    );
    const { addresses, pools, dhcp, dns, isLoading, isError, isSuccess, message } = useSelector(
        (state) => state.ip
    );

    useEffect(() => {
        if (isError) {
            console.log(message);
        }

        if (!token) {
            navigate("/login");
        }

        dispatch(getAddresses(uuid))
        dispatch(getPools(uuid))
        dispatch(getDNS(uuid))
        dispatch(getDhcp(uuid))


        return () => {
            dispatch(reset());
        };
    }, [dispatch]);

    return (
        <>
            <section className="container">
                <div className="d-flex justify-content-between flex-wrap flex-md-nowrap align-items-center pt-3 pb-2 mb-3 ">
                    <h1 className="h2">IP {isLoading ? (
                        <Spinner animation="border" role="status">
                            <span className="visually-hidden">Loading...</span>
                        </Spinner>
                    ) : ""}</h1>
                    <div className="btn-toolbar mb-2 mb-md-0">
                        <CreateIP uuid={uuid} />
                    </div>
                </div>
                <Tabs
                    defaultActiveKey="addresses"
                    transition={false}
                    id="noanim-tab-example"
                    className="mb-3"
                >
                    <Tab eventKey="addresses" title="Address List">
                        <Table className="mt-2" striped bordered hover>
                            <thead>
                                <tr>
                                    <th>Address</th>
                                    <th>Network</th>
                                    <th>Interface</th>
                                    <th>Dynamic</th>
                                    <th>Disabled</th>
                                    <th>Comment</th>
                                    <th>Actions</th>
                                </tr>
                            </thead>
                            <tbody>
                                {
                                    typeof pools === 'object' && pools !== null ?
                                        Object.keys(addresses).map((address, index) => (

                                            <tr key={index}>
                                                <td>{addresses[address]['address']}</td>
                                                <td>{addresses[address]['network']}</td>
                                                <td>{addresses[address]['interface']}</td>
                                                <td>{addresses[address]['dynamic'] == "true" ?
                                                    (<Badge bg="success">
                                                        <FaPlay size={15} />
                                                    </Badge>) : (<Badge bg="danger">
                                                        <FaStop size={15} />
                                                    </Badge>)}</td>
                                                <td>{addresses[address]['disabled'] == "false" ?
                                                    (<Badge bg="success">
                                                        Enabled
                                                    </Badge>) : (<Badge bg="warning">
                                                        Disabled
                                                    </Badge>)}</td>
                                                <td>{addresses[address]['commnet']}</td>
                                                <td>
                                                    <div>
                                                        <DeleteItem payload={{ uuid: uuid, id: addresses[address]['.id'] }} onDelete={removeAddresses} onGet={getAddresses} isLoading={isLoading} />
                                                    </div>
                                                </td>
                                            </tr>
                                        )) : ''
                                }
                            </tbody>
                        </Table>
                    </Tab>

                    <Tab eventKey="pool" title="IP Pool">
                        <Table className="mt-2" striped bordered hover>
                            <thead>
                                <tr>
                                    <th>Name</th>
                                    <th>Ranges</th>
                                    <th>Actions</th>
                                </tr>
                            </thead>
                            <tbody>
                                {
                                    typeof pools === 'object' && pools !== null ?
                                        Object.keys(pools).map((pool, index) => (

                                            <tr key={index}>
                                                <td>{pools[pool]['name']}</td>
                                                <td>{pools[pool]['ranges']}</td>
                                                <td>
                                                    <div>
                                                        <DeleteItem payload={{ uuid: uuid, id: pools[pool]['.id'] }} onDelete={removePools} onGet={getPools} isLoading={isLoading} />
                                                    </div>
                                                </td>
                                            </tr>

                                        )) : ''
                                }

                            </tbody>
                        </Table>
                    </Tab>
                    <Tab eventKey="dhcp" title="DHCP Server">
                        <Table className="mt-2" striped bordered hover>
                            <thead>
                                <tr>
                                    <th>Name</th>
                                    <th>Interface</th>
                                    <th>Lease Time</th>
                                    <th>Address Pool</th>
                                    <th>Disabled</th>
                                    <th>Actions</th>
                                </tr>
                            </thead>
                            <tbody>
                                {
                                    typeof dhcp === 'object' && dhcp !== null ?
                                        Object.keys(dhcp).map((server, index) => (

                                            <tr key={index}>
                                                <td>{dhcp[server]['name']}</td>
                                                <td>{dhcp[server]['interface']}</td>
                                                <td>{dhcp[server]['lease-time']}</td>
                                                <td>{dhcp[server]['address-pool']}</td>
                                                <td>{dhcp[server]['disabled']}</td>
                                                <td>
                                                    <div>
                                                        <DeleteItem
                                                            payload={{ uuid: uuid, id: dhcp[server]['.id'] }}
                                                            onDelete={removeDhcp}
                                                            onGet={getDhcp}
                                                            isLoading={isLoading}
                                                        />
                                                    </div>
                                                </td>
                                            </tr>

                                        )) : ''
                                }

                            </tbody>
                        </Table>
                    </Tab>

                    <Tab eventKey="vlan" title="DNS">
                        <Table className="mt-2" striped bordered hover>
                            <thead>
                                <tr>
                                    <th>Fields</th>
                                    <th>Value</th>
                                </tr>
                            </thead>
                            <tbody>

                                <tr>
                                    <td>Servers</td>
                                    <td>{typeof dns === 'object' && dns !== null ? dns[0]['servers'] : ''}</td>

                                </tr>
                                <tr>
                                    <td>Dynmic Servers</td>
                                    <td>{typeof dns === 'object' && dns !== null ? dns[0]['dynamic-servers'] : ''}</td>

                                </tr>

                            </tbody>
                        </Table>
                    </Tab>

                </Tabs>

            </section>
        </>

    )

}

export default IP;