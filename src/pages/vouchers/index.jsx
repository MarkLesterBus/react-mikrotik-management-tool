...
import { getActive, getUserProfile, getUsers, removeUser, removeUserProfile } from "../../store/voucher/methods";
import { reset } from "../../store/voucher/slice";
...
const Vouchers = () => {
    ...
    const { users, active, user_profile, isLoading, isError, isSuccess, message } = useSelector(
        (state) => state.voucher
    );
            ...
            <section className="container min-vh-100" >
                ...
                >
                    <Tab eventKey="available" title="Available">
                        {
                            typeof users === 'object' && users !== null ? <Datatable items={users.filter((user) => {
                                return !user.comment
                            })} headers={headers} items_per_page={15} /> : ''
                        }
                    </Tab>
                    ...
                    <Tab eventKey="active" title="Active">
                        {
                            typeof active === 'object' && active !== null ? <Datatable items={active} headers={[
                                { title: 'Server', value: 'server' },
                                { title: 'User', value: 'user' },
                                { title: 'Domain', value: 'domain' },
                                { title: 'Address', value: 'address' },
                                { title: 'Uptime', value: 'uptime' },
                            ]} items_per_page={10} /> : 'No Active users avaiable!'
                        }
                    </Tab>
